...
description: "Reference for common interface patterns — navigation, forms, data display (tables/lists), feedback (toasts/dialogs/empty states), and their accessibility (ARIA roles, keyboard interaction) requirements. Use when adding or reviewing a UI pattern in src/components or src/features/*/components, especially tables, dialogs, filters, and multi-step forms — pair with the base-ui skill for the underlying primitives."
category: design
tags: [ui, patterns, components, navigation, forms, accessibility, interaction-design]
version: 1.0.0
source: "https://github.com/manutej/luxor-claude-marketplace/tree/main/plugins/luxor-frontend-essentials/skills/ui-design-patterns"
---
...
Worked HTML/ARIA examples for every pattern below are in `references/examples.md`. Note: this skill's examples are framework-agnostic (plain HTML/ARIA) — adapt them to this project's Base UI + Tailwind conventions (see the `base-ui` skill) rather than copying markup directly.
